...
#define ENDSWAP(VALUE) (bigendian ? (                                                                        \
																				sizeof(VALUE) == 1 ? (VALUE) : sizeof(VALUE) == 2 ? endswap16(VALUE) \
																																	 : sizeof(VALUE) == 4		? endswap32(VALUE) \
																																	 : sizeof(VALUE) == 8		? endswap64(VALUE) \
																																													: (abort(), 1))      \
																	: (VALUE))

static enum { CACHE_CLEAN,
							CACHE_DIRTY
} cache_state;
...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		else
		{
			...
			if ((device = fopen(argv[i], "rb+")) == NULL)
			{
				perror("ERROR");
				goto cleanup;
			}
	...
	if (gpt)
	{
		if (argc >= 3)
		{
			...
			device_read(&gpt_entry,
									(ENDSWAP(gpt_header.partition_entry_lba) * lb_size) + (partition_num * sizeof(struct gpt_entry)),
		...
		else
		{
			...
			for (int64_t i = 0; i < (int64_t)ENDSWAP(gpt_header.number_of_partition_entries); i++)
			{
				struct gpt_entry gpt_entry;
				device_read(&gpt_entry,
										(ENDSWAP(gpt_header.partition_entry_lba) * lb_size) + (i * sizeof(struct gpt_entry)),
										sizeof(struct gpt_entry));
