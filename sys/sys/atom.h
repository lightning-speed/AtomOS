...
int mouseX();
int mouseY();
char isClicked(int x, int y, Component c);
void window_update(Window w);
void window_draw(Window w);
void drawRect(int x, int y, int width, int height, uint32_t color);
Window createWindow(char *name);
Component createRect(int x, int y, int width, int height, uint32_t color);
void setpixel(Window w, int x, int y, uint32_t color);
void destoryWindow(Window w);
void setOnClose(Window w, void *ef);